namespace SFRGui {
    ...
    export function createSolarGuiFor(header: string): UI.Window {
        ...
        let win: UI.Window = new UI.Window({
            ...
            elements: (() => {
                let result: UI.ElementSet = {
                    ...
                    slotCharge: {type: "slot", x: 640, y: offset + 20, bitmap: "sfr.charge_slot", size: 45, isValid: (id) => ChargeItemRegistry.isValidItem(id, "Eu", 1) || ChargeItemRegistry.isValidItem(id, "RF", 1) || ChargeItemRegistry.isValidItem(id, "FE", 1)},
                    closeButton: {type: "closeButton", bitmap: "classic_close_button", bitmap2: "classic_close_button_down", scale: 3, x: 710, y: offset}
                }
                for(let i=0; i<5; i++) result[`slotUpgrade${i}`] = {type: "slot", x: 320 + 45 * i, y: offset + 140, size: 45, isValid: (id) => SolarUpgrades.isUpgrade(id)};
